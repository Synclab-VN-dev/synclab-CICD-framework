...
(cd "$TMP/payload" && zip -qr "$TMP/test.aab" .)

keytool -genkeypair -alias synclab-test -keyalg RSA -keysize 2048 -validity 3650   -dname "CN=Synclab AAB E2E, OU=CI, O=Synclab, C=VN"   -keystore "$TMP/test.p12" -storetype PKCS12 -storepass changeit -keypass changeit >/dev/null 2>&1
...
PYTHONPATH="$ROOT" python - "$TMP/test.aab" <<'PY'
import sys
from pathlib import Path
from synclab_release.apk_verifier import verify_aab_signer
from synclab_release.errors import VerifyError
try:
    verify_aab_signer(Path("."), Path(sys.argv[1]), "CN=Synclab AAB E2E, OU=CI, O=Synclab, C=VN", "B2" * 32)
except VerifyError:
    print("PASS: wrong signer fingerprint rejected")
else:
    raise SystemExit("FAIL: wrong signer fingerprint was accepted")
PY
